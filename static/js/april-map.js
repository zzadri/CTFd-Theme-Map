const config = window.APRIL_MAP_CONFIG || {};
const root = document.querySelector(".april-map-root");
...
if (root) {
  ...
  const elements = {
    experience: document.getElementById("april-map-experience"),
    panel: document.getElementById("april-map-panel"),
    panelClose: document.getElementById("april-map-panel-close"),
    countryCode: document.getElementById("april-country-code"),
    countryName: document.getElementById("april-country-name"),
    countrySummary: document.getElementById("april-country-summary"),
    countryStatus: document.getElementById("april-country-status"),
    countryListMeta: document.getElementById("april-country-list-meta"),
    challengeList: document.getElementById("april-country-challenges"),
    filterDifficulty: document.getElementById("april-filter-difficulty"),
    filterCategory: document.getElementById("april-filter-category"),
    filterReset: document.getElementById("april-filter-reset"),
    mapViewport: document.getElementById("april-map-viewport"),
  ...
  boot().catch(error => {
    ...
    if (elements.countryStatus) {
      ...
      elements.countryStatus.className = "april-map-panel-status is-danger";
    }
  });
  ...
  function removeModalRuntimeStylesheet() {
    document.getElementById("april-modal-light-runtime")?.remove();
  }

  function ensureModalRuntimeStylesheet() {
    const existing = document.getElementById("april-modal-light-runtime");
    if (existing) {
      return;
    }

    const style = document.createElement("style");
    style.id = "april-modal-light-runtime";
    style.textContent = [
  ...
  function renderCountry(countryCode) {
    ...
    if (elements.countryStatus) {
      ...
      elements.countryStatus.className = state.loadError
        ? "april-map-panel-status is-warning"
        : "april-map-panel-status";
    }
  ...
  function renderCountryChallenges(entries, totalCount) {
    ...
    entries.forEach((entry, index) => {
      ...
      row.type = "button";
      row.className = `april-map-challenge-row${solved ? " is-solved" : ""}${missing ? " is-missing" : ""}`;
      row.disabled = missing;
      row.setAttribute("aria-label", `${missing ? "Indisponible" : "Ouvrir"} ${heading}`);
      row.innerHTML = `
        <span class="april-map-challenge-index">${ordinal}</span>
        <div class="april-map-challenge-main">
          <p class="april-map-challenge-name">${escapeHtml(heading)}</p>
          <div class="april-map-challenge-tags">
            <span class="april-map-tag april-map-tag-category">${escapeHtml(categoryLabel)}</span>
            ${renderTags(visibleTags)}
          </div>
        </div>
        <div class="april-map-challenge-side">
          <span class="april-map-challenge-points">${escapeHtml(pointsLabel)}</span>
          <span
            class="april-map-challenge-difficulty ${difficulty ? difficulty.tone : "is-unknown"}"
  ...
  function renderTags(tags) {
    return tags.map(tag => `<span class="april-map-tag">${escapeHtml(tag)}</span>`).join("");
  }
